...
export const getCodeForcesDetails = async (
  username: string,
  token: string
) => {
  const toastId = toast.loading("Fetching your CodeForce stats...");
  let result = [];
  ...
  try {
    ...
    const res = await fetch(`/api/platform/getCodeForcesDetails/${username}`, {
  method: "GET",
  headers: {
    "Content-Type": "application/json",
    Authorization: `Bearer ${token}`,
...
console.log("CodeForces Details Response:", res);
let data;
try {
  const text = await res.text();
  data = text ? JSON.parse(text) : {};
...
if (!res.ok || !data.success) {
  throw {
    ...
    toast.update(toastId, {
      render: "CodeForces stats fetched successfully!",
      type: "success",
      isLoading: false,
      autoClose: 3000,
    });
  } catch (error: any) {
    console.error("GET_CODEFORCES_DETAILS ERROR >>>", error);
    toast.dismiss(toastId);

    let errorMessage = "Could not fetch CodeForces details. Please try again.";
